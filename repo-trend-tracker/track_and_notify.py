...
def load_watchlist_seed():
    if os.path.exists(WATCHLIST_SEED):
...
def discover_candidates():
    candidates = (
        ...
        | repos_from_search()
        | load_watchlist_seed()
        | previously_tracked_repos()  # keep tracking repos we already started on
    )
    return list(candidates)[:MAX_CANDIDATES_PER_RUN]
...
def load_history():
    """full_name -> sorted list of snapshot dicts"""
    history = {}
    if not os.path.exists(SNAPSHOTS_CSV):
        return history
    with open(SNAPSHOTS_CSV) as f:
...
def compose_html(fast_growers, reasons, overlap, dashboard_url, errors):
    ...
    if errors:
        parts.append(f"<p style='color:red;'>⚠️ Issues: {', '.join(errors)}</p>")
...
def main():
    ...
    if not candidates:
        errors.append("discovery returned nothing")
    ...
    if to_notify or not fast_growers:
        html = compose_html(to_notify, reasons, overlap, dashboard_url, errors)
        send_email(html)
